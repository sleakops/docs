...
3. **Establecer Tolerations**: **Crítico** - Actualiza todos los campos `toleration` en los valores del chart para apuntar a tu NodePool
...
:::tip Importante
Asegúrate de que cada campo `toleration` en los valores del chart esté configurado correctamente para usar un NodePool. Sin esta configuración, Kubernetes no puede determinar dónde programar los pods, lo que lleva a fallas en el despliegue.
:::

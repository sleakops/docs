...
sidebar_position: 2
title: "Django + Celery"
description: "Learn how to deploy your Django + Celery project usign Sleakops"
tags:
  - django
  - celery
  - deploy
...
### 🎛️ **Total Control and Customization**
...
- A Sleakops account
- A cluster in this account. If you don't have one, [here's the documentation on how to create one](/cluster).
- A configured environment. If you don't have one, [here's the documentation on how to create one](/environment).
...
For more information, see our [workloads documentation](/project/workload)
...
You can switch between "production" and "non-production" environments. This gives you default configuration values ​​in the next step for each environment. For example, in a production environment, it enables multi-A-Z, automatic backups, etc.
...
| \*\_POSTGRESSQL_PORT | DB_POSTGRESDB_PORT |
...
We're going to create a new variable group. We'll switch to text mode to copy the missing variables from the .env.example file and adjust the values ​​accordingly.
...
- **Read Replicas**: Use read replicas for reporting and analytics queries. (This can be done from Sleakops in the [Postgres](/project/dependency/postgresql-aws) configuration.)
- **Indexing**: Optimize database indexes for workflow execution queries.
- **Backup Strategies**: Implement automated backups with point-in-time recovery. (This can be done from Sleakops in the [Postgres](/project/dependency/postgresql-aws) configuration.)

...
4. **Fail:** Errors appear when a request lands on a different replica than the one that initiated a session (sticky-session issue) — the root cause is session state kept in the Pod's memory instead of a shared backend; [sticky sessions](/tutorial/sticky-sessions) can mitigate it while you externalize that state

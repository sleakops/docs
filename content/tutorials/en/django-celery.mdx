...
sidebar_position: 10
description: Learn how to deploy your Django + Celery project with distributed and scalable worker mode on Kubernetes using Sleakops
tags:
  - django
  - celery
  - deploy
  ...
  - kubernetes
  - scaling
image: ../django-celery/django-celery.png
---
...
- An Environment configured. If you don't have it, [here is the documentation on how to do it](/docs/environment)
...
In step 3 we have to edit the path field and put the endpoint of healthcheck which
in our case is “/healthcheck/”. Then click on the “Next” button until the web service
is created:
<Zoom overlayBgColorEnd="rgba(255, 255, 255, 0.8)">
  <img
...
In the first form we will have to select our project and define a name for it. Then
...
In the following form we have several fields but the only ones that matter to us
for this example are the username and password, we can define whatever we want. For
this example I chose admin as username and for the password I generated it randomly
with the dice button. Then we click on the “Next” button to go to the next form:
<Zoom overlayBgColorEnd="rgba(255, 255, 255, 0.8)">
  <img
...
This concludes our project deployment process. We leave you an optional step which
is to configure the ci with github.

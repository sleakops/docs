...
4. Dejá **Deploy?** activado y guardá. La anotación llega al cluster en el próximo deploy.

{/* TODO: screenshot - diálogo IngressConfig con la anotación target-group-attributes cargada en la tabla de Extra Annotations */}
...
### Configuralo al crear el Web Service

El mismo campo está disponible en el alta. En el paso **Define the connection**, expandí la sección **Advanced** y completá **Extra Annotations** dentro de **Ingress Annotations**. La anotación se aplica a todos los hosts del Web Service: su URL por defecto y cualquier Domain propio que apunte a él.

{/* TODO: screenshot - paso Define the connection con la sección Advanced expandida, mostrando Ingress Annotations */}
...
Cuando termine el deploy, revisá las tres capas:
...
Con `lb_cookie` deberías ver una cookie `AWSALB` (más `AWSALBCORS` en requests cross-origin). Con `app_cookie` el load balancer agrega `AWSALBAPP` junto a tu propia cookie. Si no hay ninguna cookie, la anotación no llegó al Ingress: volvé al primer paso y confirmá que el deploy terminó.
...
**3. La anotación está en el objeto real.** En Headlamp, abrí el Ingress del Namespace de tu Environment y verificá que `alb.ingress.kubernetes.io/target-group-attributes` aparezca en sus anotaciones con el valor que cargaste.

{/* TODO: screenshot - Headlamp mostrando las anotaciones del Ingress, incluida target-group-attributes */}

...
4. **Deploy**: dejá **Deploy?** activado para desplegar el cambio de inmediato, o desactivalo para guardarlo como cambio pendiente
...
## Validación de una Dependencia

Agregar, editar o eliminar una dependencia dispara una validación en segundo plano del chart de tu proyecto (eliminar la última se saltea la validación). Si falla, tu proyecto pasa a estado Error y recibís una notificación con el error específico — abrí **Chart Configuration** desde la notificación para corregir los values de la dependencia, o eliminarla (y volver a agregarla con otra versión si hace falta). El proyecto se recupera automáticamente en cuanto el chart valida correctamente.

## Editar y Eliminar una Dependencia

Usá la acción **Edit** de una dependencia para actualizar sus values (el nombre del chart, el repositorio y la versión no se pueden cambiar después de crearla — para cambiar la versión, eliminá la dependencia y agregala de nuevo). **Delete** la elimina y dispara un nuevo deploy para sacarla del clúster.
...
1. Asegúrate de que el EBS CSI Driver esté instalado y funcionando en tu clúster. Puedes consultar la [documentación de Addon](/docs/cluster/addons/ebs) para obtener orientación sobre cómo configurar EBS.
...
persistence:
  ...
  size: 5Gi
```
3. En caso de que necesites otro storageClass, puedes definirlo como un [extra template](/docs/project/chart/extra_templates) y usarlo. Recuerda configurar el provisioner como `ebs.csi.aws.com`.
...
1. Asegúrate de que el EFS CSI Driver esté instalado y funcionando en tu clúster. Sleakops instala este addon cuando creas un [volumen](/docs/project/volumes) para un proyecto. Puedes crear y luego eliminar un volumen para completar la instalación.
...
persistence:
  ...
  size: 5Gi
```
3. En caso de que necesites otro storageClass, puedes definirlo como un [extra template](/docs/project/chart/extra_templates) y usarlo. Recuerda configurar el provisioner como `efs.csi.aws.com`.

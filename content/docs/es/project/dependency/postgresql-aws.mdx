...
Sí, editá la dependencia y elegí una nueva **Versión del Motor de Base de Datos**. Actualizar a una nueva versión mayor se trata como un cambio potencialmente destructivo: SleakOps lo retiene en "Awaiting Approval" para que lo confirmes antes de aplicarlo.
</details>
...
<details>
<summary>
### ¿Cómo puedo crear un backup (dump) de mi base de datos PostgreSQL?
</summary>
Para crear un backup de tu base de datos PostgreSQL:
...
<details>
<summary>
### ¿Cómo importo un backup existente usando Docker?
</summary>
...
Sí, editá la dependencia y quitala de la lista **Definición de Réplicas de Lectura de RDS**.
</details>

<details>
<summary>
### ¿SleakOps puede restaurar un dump automáticamente?
</summary>
Sí. Desde el menú **Actions** de la dependencia, ejecutá **Restore Database**: indicá un archivo de dump en S3 y elegí la base de datos PostgreSQL de destino (cualquier dependencia del mismo Project-Environment que esté en estado `created`, incluso una distinta de la que abriste el menú). SleakOps corre la restauración como un Job y podés seguir su progreso desde *Jobs*.
</details>
...
Para más detalles, consulta [Dependencies: Integración de Bases de Datos, Caching y Servicios de Mensajería](/docs/project/dependency).
...
| --- | --- |
| **Nombre de Base de Datos** | Nombre de la base de datos por defecto creada dentro de la instancia. No puede ser `db` ni `database`. |
| **Versión del Motor de Base de Datos** | Selecciona la versión específica del motor de base de datos PostgreSQL que deseas usar. Esto asegura la compatibilidad con los requisitos de tu aplicación. Ejemplo: `PostgreSQL 16`, `PostgreSQL 18`. |
...
| **Almacenamiento de Base de Datos** | Especifica la cantidad de almacenamiento asignado para la base de datos, en GiB. |
| **Storage Autoscaling Enabled** | Permite que AWS aumente el almacenamiento automáticamente a medida que se llena, hasta un **Maximum Allocated Storage** que definas. |
...
| **Contraseña** | Contraseña para el usuario principal para acceder a la base de datos, o generala automáticamente. |
...
| **Create a RDS from a snapshot** | Restaura la nueva base de datos desde un snapshot de RDS existente en vez de arrancar vacía. |
...
Después de estos datos básicos, necesitarás decidir si se creará una réplica. Las réplicas también se pueden agregar, editar o quitar más adelante desde el formulario de edición. Para hacerlo:

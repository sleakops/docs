...
| **Autenticación de Cliente** | *(Requerido para modo Provisioned)* Cómo se autentican los clientes ante el clúster: `Unauthorized` (solo cifrado TLS), `IAM` (SASL/IAM usando roles de AWS IAM), o `SCRAM` (SASL/SCRAM-SHA-512 con usuario y contraseña). |

...
Puedes definir los volúmenes en el formulario del [Proyecto](/docs/project):
<Zoom overlayBgColorEnd="rgba(255, 255, 255, 0.8)">
  <img src="/img/project/PROJECT-volumes-form.png" alt="PROJECT-volumes-form" style={{ height: '250px', width: 'auto' }}/>
</Zoom>

Cada vez que se necesite un volumen, Sleakops despliega un EFS CSI Driver dentro del [clúster](/docs/cluster). Esto permite que cada Proyecto tenga su propio montaje único de volumen, almacenando carpetas basadas en las rutas que especifiques.
...
Para más detalles sobre cómo se usa EFS para volúmenes, consulta la [documentación de EFS](/docs/cluster/addons/efs).
...
3. **Establece la capacidad de almacenamiento** - Define la cantidad de espacio de almacenamiento asignado al volumen (1–1000 GB)
4. **Elige un filesystem** - Selecciona qué filesystem EFS, de los configurados en el addon EFS del clúster, respalda el volumen
5. **Elige la política de retención** - Selecciona entre:
...
- La capacidad de almacenamiento se especifica en GB, de 1 a 1000
...
Eliminar un volumen dispara un redeploy que lo desmonta de los pods de tu proyecto (el switch de Deploy del diálogo de eliminación viene activado por defecto); si lo desactivas, el volumen se desmonta en el próximo deploy del proyecto.

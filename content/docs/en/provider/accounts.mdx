...
Once the Accounts are up, we set to each one of them what we call [Network Module](/network)  it contains a lot of different AWS services that are used to make the network connections inside accounts.

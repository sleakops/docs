...
Access _Clusters_, select one and click the button: $
</details>
...
- **Data Corruption:** A bug in Staging code could trigger accidental commands (such as `DROP TABLE` or `DELETE`) that directly impact live Production data.
- **Credential Compromise:** If an attacker compromises the Staging environment (which often has more relaxed security policies), they could escalate privileges to gain access to critical Production credentials or master keys.
</details>
...
Into the _Left Pane_, access _Clusters_ option and then, at the top right corner, click on the _Create_ button.
...
With your _Account_ selected, you will access the following form:
...
Once you've completed the form, click on _Submit_ in order to trigger the cluster creation into the selected account on AWS.

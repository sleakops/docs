...
Custom addons (deployed from your own Helm chart, not one of SleakOps' managed types) can move in and out of SleakOps in two ways: importing a JSON definition exported from another cluster, or discovering and adopting a Helm release already running in the cluster. Both are reached from the Addons list next to **Add custom addon**, and both are only available when your company has the import feature enabled (`infra_import.addon`) and you aren't a viewer.
...
This pairs with **Export JSON**, available on an installed custom addon's detail view (same access requirement, addon in `created` state). It copies the addon's portable JSON to the clipboard and downloads it as `<addon-name>.addon.json` — that file is what you paste into **Import from JSON** on the destination cluster.

...
When you create an Aurora MySQL dependency in SleakOps, it automatically generates a Vargroup for your database cluster. This Variable Group securely stores the Aurora MySQL credentials and other important configuration details, such as the cluster endpoint and user access information. You'll be able to manage them from the [Vargroups](/docs/project/vargroup) section.
</details>
...
| **Read Replicas** | Configuration for database read replicas. Each replica requires a name and publicly accessible setting; replicas can be added, edited, or removed later from the Edit form too. |

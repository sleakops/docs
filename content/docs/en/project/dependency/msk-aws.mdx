...
| **Client Authentication** | *(Required for Provisioned mode)* How clients authenticate to the cluster: `Unauthorized` (TLS encryption only), `IAM` (SASL/IAM using AWS IAM roles), or `SCRAM` (SASL/SCRAM-SHA-512 with a username and password). |

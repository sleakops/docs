...
- [Releases](/project/deployment/release.mdx): These are versioned combinations of your [Workloads](/project/workload) including WebServices, Workers, CronJobs, and Hooks.
- Build Image: Represented by a [Build](/project/build).
- Configuration Values: These comprise resources like [Volumes](/project/volumes.mdx) and [VariableGroups](/project/vargroup).
...
Various methods exist to generate a Deployment. These are outlined below. Note that we force certain Deployments. For details, refer to [More on Deployment](/project/deployment/more_on_deployment.mdx#forced-deployments).

- [Workloads](/project/workload/index.mdx): Provides a switcher allowing you to decide whether to run a new Deployment.
- [VariableGroup](/project/vargroup): Operates similarly to Workloads but doesn't create a new Release. Instead, it only updates the [values](https://helm.sh/docs/chart_best_practices/values/) of the Deployment.
- [Dependency](/project/dependency): Triggers a Deployment automatically. Delve deeper into this at [More on Deployment](/project/deployment/more_on_deployment.mdx).

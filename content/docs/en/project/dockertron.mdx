...
To start the dockerization process, choose the private repository and branch to analyze, then answer a short questionnaire about your project so the AI has enough context. Only private repositories from your connected Git integration are available.
...
The system sends a detailed error message indicating what went wrong. You can start a new execution with adjusted information (framework, dependencies, or special configuration) so the AI agent can generate the correct configuration.

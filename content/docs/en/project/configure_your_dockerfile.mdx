...
You can add or update them by following the steps below.
</details>
...
| --- | --- |
  ...
  :::tip
    If you choose to add the arguments using the **Textmode** option: add each argument on its own line in `KEY=VALUE` format (spaces around the equal sign are ignored). For multiline values, such as private keys, wrap the value in double quotes. 

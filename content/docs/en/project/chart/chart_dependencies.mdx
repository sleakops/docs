...
Adding, editing, or removing a dependency triggers a background validation of your project's chart. If it fails, your project moves to an Error state and you get a notification with the specific error — open **Chart Configuration** from the notification to fix the dependency's version or values, or delete it. The project recovers automatically once the chart validates successfully.

## Editing and Removing a Dependency

Use a dependency's **Edit** action to update its version or values (the chart name and repository can't be changed after creation). **Delete** removes it and triggers a new deploy to take it out of the cluster.

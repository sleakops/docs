...
## Validating a Dependency

Adding, editing, or removing a dependency triggers a background validation of your project's chart (removing the last one skips it). If it fails, your project moves to an Error state and you get a notification with the specific error — open **Chart Configuration** from the notification to fix the dependency's values, or delete it (and re-add it with a different version if needed). The project recovers automatically once the chart validates successfully.

## Editing and Removing a Dependency

Use a dependency's **Edit** action to update its values (the chart name, repository, and version can't be changed after creation — to change the version, delete the dependency and add it again). **Delete** removes it and triggers a new deploy to take it out of the cluster.

...
<details>
<summary>
### Does Autodiagnostics check for Kubernetes upgrade compatibility?
</summary>
Yes, for **EKS Cluster Upgrade** migrations (see [Upgrades](/docs/upgrades)). Their detail view includes an **Unmanaged Manifests Readiness** check: it reviews resources you deployed outside SleakOps — custom Helm releases and extra manifests — against the Kubernetes version you're upgrading to, and flags anything relying on deprecated or removed APIs, with a recommended action for each.

This check is specific to EKS Cluster Upgrade migrations — other migration types don't include it.
</details>

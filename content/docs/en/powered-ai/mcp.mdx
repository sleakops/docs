...
Creating or reconfiguring the platform's own infrastructure is not available through the MCP — clusters, node pools, cluster addons, environments, providers and AWS accounts. Those operations cost money and reshape your AWS footprint, so they stay a deliberate decision in the console. The agent can still **read** all of them to answer questions and diagnose.
...
<details>
  ...
  Tools that create or reconfigure clusters, node pools, cluster addons, environments, providers or AWS accounts are intentionally not exposed. Do it in the console; the agent can read those resources without restrictions.
</details>

...
- If the ALIAS is a direct subdomain of a domain already managed by SleakOps, it reuses that domain's certificate automatically — you still need to create the CNAME record pointing to the load balancer.
...
#### Create an Alias for a web service workload and delegate it 
  ...
  d. Create the Alias if it doesn’t exist by clicking the Associate new Domain and completing the form (the form includes a deploy option, checked by default, that redeploys the service immediately to start serving the new domain — uncheck it to queue the change for later approval instead).
...
If your alias is a direct subdomain of a domain already managed by SleakOps, it reuses that domain's certificate instead of issuing its own — the panel currently shows this as "Cert. Pending" regardless of whether the parent's certificate is already active. Check the parent domain's status to confirm the certificate is valid.
</details>

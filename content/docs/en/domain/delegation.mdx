...
✅ **No action needed** - delegation is automatic
...
graph LR
    ...
    B --> C[⚠️ Manual<br/>NS Records Required]
...
✅ **No delegation needed** - Sleakops automatically:
...
Since `api.external.com` is a direct subdomain of a domain Sleakops already manages, it reuses that domain's certificate — there's no separate SSL validation step.
...
💡 **Tip:** Use `https://dnschecker.org` to check DNS propagation globally

...
A cluster must be selected to create an environment, hence that according to our [proposed architecture](/provider/aws.mdx) the prod account must only contain production environments.
...
- A Kubernetes job to perform the [initial build image](/project/initial-build.mdx).

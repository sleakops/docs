...
When you create a Sleakops deployment a Helm release will be installed in your [cluster](/cluster) using a Helm chart built when a [release](/deployment/release.mdx) is created
A Kubernetes deployment will pull an image (named as: environment_name-latest), built in [initial-build](/project/build/initial-build), from the corresponding image repository (AWS ECR) built in [project](/project)

...
To create a deployment for a project in an environment you need to provide a [environment](/docs/environment), [project](/docs/project) and a [release](/docs/deployment/release)

## How SleakOps handle deployments

When you create a Sleakops deployment a Helm release will be installed in your [cluster](/docs/cluster) using a Helm chart built when a [release](/docs/deployment/release.mdx) is created
A Kubernetes deployment will pull an image (named as: environment_name-latest), built in [initial-build](/docs/project/initial-build), from the corresponding image repository (AWS ECR) built in [project](/docs/project)
and desired numbers of Kubernetes pods will be created, running the desired image. A Kubernetes service and ingress will be created to allow the creation of the corresponding subdomains

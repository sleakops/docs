...
- **Minimum resources per Nodepool node**: Now each Nodepool, within its advanced configurations, allows you to define minimum resource values that the Nodepool requests per node.
- **Job with specific images**: When creating a Job, you can now specify the exact image and tag you want to run (for example, `postgres:16.4`).
- **(BETA) Chart dependencies per Project**: It’s now possible to add *Dependencies* to the Chart deployed by SleakOps from your Project configuration. This triggers the same behavior described in the [Helm documentation](https://helm.sh/docs/helm/helm_dependency/).

**🐞 Bug Fixes:**

- **Internal Web Service URL**: Fixed an issue that caused the URL for “internal” Web Services to be incorrect.
- **Volume deletion**: Addressed certain problems with deleting Project Volumes that prevented them from being successfully removed.
- **CI/CD file simplification**: The CI/CD file shown per Project was simplified due to the removal of a minor, unneeded step.

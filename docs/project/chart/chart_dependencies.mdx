...
1. Ensure that the EBS CSI Driver is installed and functioning in your cluster. You can refer to the [Addon documentation](/cluster/addons/ebs) for guidance on setting up EBS.
2. Ensure you completed the values with the appropiate storageClass that is created with the EBS addon. For example a persistence value for a chart could look like this:
```yaml
persistence:
  enabled: true
  ...
  size: 5Gi
```
3. In case you need another storageClass, you can define it as an [extra template](/project/chart/extra_templates) and use it. Remember to set the provisioner to `ebs.csi.aws.com`.
...
1. Ensure that the EFS CSI Driver is installed and functioning in your cluster. Sleakops installs this addon when you create a [volume](/project/volumes) for a project. You can create and then delete a volume to complete the installation.
2. Ensure you completed the values with the appropiate storageClass that you  prefer for this data. Sleakops creates two StorageClasses when installing the EFS addon: `efs-sc-retain` and `efs-sc-delete`. For example a persistence value for a chart could look like this:
```yaml
persistence:
  enabled: true
  ...
  size: 5Gi
```
3. In case you need another storageClass, you can define it as an [extra template](/project/chart/extra_templates) and use it. Remember to set the provisioner to `efs.csi.aws.com`.

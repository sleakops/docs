...
3. **Set Tolerations**: **Critical** - Update all `toleration` fields in the chart values to target your NodePool
...
:::tip Important
Ensure every `toleration` field in the chart values is properly configured to use a NodePool. Without this configuration, Kubernetes cannot determine where to schedule the pods, leading to deployment failures.
:::

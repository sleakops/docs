...
according to our [proposed architecture](/provider/aws.mdx). The resulting image has the project name and is tagged `environment-name_latest`.

## Summary of resources created in the cluster

As previously discussed when the Kaniko job is created, all the resources listed below are also created inside the respective cluster:
